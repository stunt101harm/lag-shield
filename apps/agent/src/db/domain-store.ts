  ...
  for (const outcome of event.payload.outcomes) {
    await sql`
      INSERT INTO outcome_quote_observations (
        ...
        ${'reportedProbabilityMicros' in outcome ? outcome.reportedProbabilityMicros : null},
        ${event.payloadVersion === 2 ? event.payload.probabilityEncoding : null},
        ${event.source},
        ${event.sourceId},
...
async function applyScoreProjection(
  ...
  await sql`
    INSERT INTO score_events (
      ...
      ${event.fixtureId},
      ${payload.action},
      ${'actionId' in payload ? payload.actionId : null},
      ${'confirmed' in payload ? payload.confirmed : null},
      ${'details' in payload ? jsonParameter(toJsonValue(payload.details)) : null}::jsonb,
      ${payload.period},
      ${payload.statusId},
      ${payload.homeScore},
      ${payload.awayScore},
